...
const formatDateTime = (value) => value
    ...
    : 'Không có';
function renderEmailLayout({ title, intro, contentHtml, ctaLabel, ctaUrl, }) {
...
function bookingRows(data, status, includeContact = false) {
    return [
        ['Mã booking', data.bookingCode],
        ['Villa', `${data.villaName}${data.villaLocation ? ` — ${data.villaLocation}` : ''}`],
        ...(includeContact ? [
            ['Khách', data.guestName || 'N/A'],
            ['Số điện thoại', data.guestPhone || 'N/A'],
            ['Email khách', data.guestEmail || 'N/A'],
        ] : []),
...
function bookingTextRows(data, status, includeContact = false) {
    return bookingRows(data, status, includeContact).map(([label, value]) => `${label}: ${value || 'N/A'}`);
}
function textFooter(ctaLabel, ctaUrl) {
    return [
