...
router.post('/', async (req, res, next) => {
    try {
        ...
        if (!bookingCode)
            throw new errors_1.AppError(400, 'VALIDATION_ERROR', 'bookingCode là bắt buộc.');
        if (!phone)
            throw new errors_1.AppError(400, 'VALIDATION_ERROR', 'phone là bắt buộc.');
        if (!Number.isInteger(rating) || rating < 1 || rating > 5)
            throw new errors_1.AppError(400, 'VALIDATION_ERROR', 'rating phải từ 1 đến 5.');
        if (comment && comment.length > 1000)
            throw new errors_1.AppError(400, 'VALIDATION_ERROR', 'comment tối đa 1000 ký tự.');
        const booking = await prisma_1.prisma.booking.findFirst({ where: { bookingCode, guestPhone: phone }, include: { feedback: true } });
        if (!booking)
            throw new errors_1.AppError(404, 'BOOKING_NOT_FOUND', 'Không tìm thấy booking.');
        if (booking.status !== 'confirmed' && booking.status !== 'completed')
            throw new errors_1.AppError(403, 'FEEDBACK_NOT_ALLOWED', 'Booking chưa đủ điều kiện đánh giá.');
        if (booking.checkOut.getTime() >= Date.now())
            throw new errors_1.AppError(403, 'FEEDBACK_NOT_ALLOWED', 'Chỉ có thể đánh giá sau ngày check-out.');
        if (booking.feedback)
