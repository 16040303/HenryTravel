...
export default function ImageUploader({ value, onChange, maxFiles = 20, disabled = false }: ImageUploaderProps) {
  ...
  const handleFilesSelected = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (value.length + selectedFiles.length > maxFiles) {
      setError(t('admin.upload.maxFiles', { maxFiles }));
      return;
    }

    const invalidType = selectedFiles.find((file) => !allowedTypes.has(file.type));
    if (invalidType) {
      setError(t('admin.upload.invalidType'));
      return;
    }

    const oversizedFile = selectedFiles.find((file) => file.size > MAX_FILE_SIZE_BYTES);
    if (oversizedFile) {
      setError(t('admin.upload.tooLarge'));
      return;
    }

    try {
      setIsUploading(true);
      const response = await uploadAdminMedia(selectedFiles);
      onChange([...value, ...response.files.map((file) => file.secureUrl || file.url)]);
    } catch (uploadError) {
      setError(uploadError instanceof Error ? uploadError.message : t('admin.upload.failed'));
    } finally {
  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
        <div>
          <span className="text-[10px] font-bold text-neutral-400 uppercase">{t('admin.upload.label')}</span>
          <p className="text-[10px] text-neutral-400 mt-0.5">
            {t('admin.upload.help', { maxFiles })}
          </p>
        </div>
        <button
          type="button"
        ...
        >
          ...
          <span>{isUploading ? t('admin.upload.loading') : t('admin.upload.button')}</span>
        </button>
      </div>
      ...
      {value.length > 0 ? (
          ...
          {value.map((url, index) => (
            <div key={`${url}-${index}`} className="relative overflow-hidden rounded-2xl border border-neutral-100 bg-neutral-50 shadow-sm group">
              ...
              <button
                ...
                title={t('admin.upload.removeTitle')}
              >
              ...
              <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/60 to-transparent p-2 text-[9px] font-black text-white">
                {t('admin.upload.imageLabel', { index: index + 1 })}
              </div>
            </div>
          ))}
        </div>
        ...
        >
          ...
          <span className="text-[11px] font-bold text-[#0071c2]">{t('admin.upload.choose')}</span>
          <span className="text-[9px] text-neutral-400 font-semibold">{t('admin.upload.note')}</span>
        </button>
      )}
    </div>
  );
